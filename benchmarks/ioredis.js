...
if (cluster.isMaster) {
    ...
    worker.on('message', (event)=>{
      ...
      if(event.msg === 'finish') {
        if(++finished === numCPUs) {
          // calc stats
          console.log('==========================');
          console.log(require('../node_modules/ioredis/package.json').name+': ' + require('../node_modules/ioredis/package.json').version);
  ...
  var redis = new Redis.Cluster([{ host: '127.0.0.1', port: 7001}]);
  redis.on('ready', ()=>{
